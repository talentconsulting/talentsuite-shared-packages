...
{
    /// <summary>
    /// use this in calling apps:
    ///  apiUrl = Configuration.GetSection("apiUrl").Value;
    ///  services.AddScoped(n => new ApiClient(apiUrl));
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class ApiClient<T> : HttpClient, IApiClient<T> where T : class
    {
        ...
        public async Task Create(T item)
        {
            try
            {
                ...
                var response = await PostAsync(apiUrl, bodyContent);
                if (!response.IsSuccessStatusCode)
                {
                    throw new Exception
        ...
        public async Task Update(int? id, T item)
        {
            try
            {
                ...
                var response = await PutAsync(apiUrl + $"/{id}", bodyContent);
        ...
        public async Task Delete(int id)
        {
            try
            {
                ...
                var response = await DeleteAsync(apiUrl + $"/{id}");
